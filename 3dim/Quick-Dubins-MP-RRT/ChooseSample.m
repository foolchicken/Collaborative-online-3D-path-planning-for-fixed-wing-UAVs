...
if ~isinf(a)  % 如果已经找到路径，在椭球中进行搜索
    ...
    qs = transform(p, a, b, c, [0,0,algoParam.start(3)], goalxyh);
else
    if rand < 0.1
    ...
    qs = [LL2XY(pos, algoParam.start), rand() * (algoParam.dynamicCons.maxh - ht - algoParam.dsafe) + ht + algoParam.dsafe];
    %qs = [LL2XY(pos, algoParam.start), rand() * (algoParam.dynamicCons.maxh - ht - algoParam.dsafe) + ht + algoParam.dsafe];
end
