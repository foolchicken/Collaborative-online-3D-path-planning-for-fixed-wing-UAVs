...
if ifconsAnglef
    mp2dim = GetMotionPrimitive(dataBase, [startPose(1 : 2) startPose(4)], [goalPose(1 : 2) goalPose(4)]);
else
    mp2dim = GetMotionPrimitiveTF(dataBase, [startPose(1 : 2) startPose(4)], [goalPose(1 : 2) 0]); % 不约束终端角，查询的时候方位角设为0
end
...
motionPrimitive.path = path(:, 1 : 3) + startPose(1 : 3);
