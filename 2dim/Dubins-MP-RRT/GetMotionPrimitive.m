...
startPose(3) = wrapTo2Pi(startPose(3)); % 角度规范化
goalPose(3) = wrapTo2Pi(goalPose(3));
...
elseif goalPose(1) < 0 && goalPose(2) >= 0 % 第二象限
    theta0ind = wrapTo2Pi(pi - startPose(3)) / thetares + 1;
    thetafind = wrapTo2Pi(pi - goalPose(3)) / thetares + 1;
    xind = -goalPose(1) / gridres + 1;
    yind = goalPose(2) / gridres + 1;
elseif goalPose(1) < 0 && goalPose(2) < 0 % 第三象限
    theta0ind = wrapTo2Pi(startPose(3) - pi) / thetares + 1;
    thetafind = wrapTo2Pi(goalPose(3) - pi) / thetares + 1;
    xind = -goalPose(1) / gridres + 1;
    yind = -goalPose(2) / gridres + 1;
end
...
motionPrimitive.theta0 = wrapTo2Pi(motionPrimitive.theta0);
motionPrimitive.thetaf = wrapTo2Pi(motionPrimitive.thetaf);
